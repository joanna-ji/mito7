 ...
 * - sizeOfZonesInAcre
 */
public final class MitoModel2017withMoped {

    private static final Logger logger = Logger.getLogger(MitoModel2017withMoped.class);
    private final String scenarioName;

    private DataSet dataSet;

    private MitoModel2017withMoped(DataSet dataSet, String scenarioName) {
        this.dataSet = dataSet;
        this.scenarioName = scenarioName;
        MitoUtil.initializeRandomNumber();
    }

    public static MitoModel2017withMoped standAloneModel(String propertiesFile, ImplementationConfig config) {
        logger.info(" Creating standalone version of MITO ");
        Resources.initializeResources(propertiesFile);
        MitoModel2017withMoped model = new MitoModel2017withMoped(new DataSet(), Resources.instance.getString(Properties.SCENARIO_NAME));
        model.readStandAlone(config);
        return model;
    }

    public static MitoModel2017withMoped initializeModelFromSilo(String propertiesFile, DataSet dataSet, String scenarioName) {
        logger.info(" Initializing MITO from SILO");
        Resources.initializeResources(propertiesFile);
        MitoModel2017withMoped model = new MitoModel2017withMoped(dataSet, scenarioName);
    ...
    public void run() {
        ...
        TravelDemandGenerator2017withMoped ttd = new TravelDemandGenerator2017withMoped.Builder(dataSet).build();

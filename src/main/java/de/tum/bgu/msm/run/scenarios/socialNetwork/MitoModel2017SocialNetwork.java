 ...
 * - sizeOfZonesInAcre
 */
public final class MitoModel2017SocialNetwork {

    private static final Logger logger = Logger.getLogger(MitoModel2017SocialNetwork.class);
    private final String scenarioName;

    private DataSet dataSet;

    private MitoModel2017SocialNetwork(DataSet dataSet, String scenarioName) {
        this.dataSet = dataSet;
        this.scenarioName = scenarioName;
        MitoUtil.initializeRandomNumber();
    }

    public static MitoModel2017SocialNetwork standAloneModel(String propertiesFile, ImplementationConfig config) {
        logger.info(" Creating standalone version of MITO ");
        Resources.initializeResources(propertiesFile);
        MitoModel2017SocialNetwork model = new MitoModel2017SocialNetwork(new DataSet(), Resources.instance.getString(Properties.SCENARIO_NAME));
        model.readStandAlone(config);
        return model;
    }

    public static MitoModel2017SocialNetwork initializeModelFromSilo(String propertiesFile, DataSet dataSet, String scenarioName) {
        logger.info(" Initializing MITO from SILO");
        Resources.initializeResources(propertiesFile);
        MitoModel2017SocialNetwork model = new MitoModel2017SocialNetwork(dataSet, scenarioName);
    ...
    public void run() {
        ...
        TravelDemandGeneratorSocialNetwork ttd = new TravelDemandGeneratorSocialNetwork.Builder(dataSet).build();

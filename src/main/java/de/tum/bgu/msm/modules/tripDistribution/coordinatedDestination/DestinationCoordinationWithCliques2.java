...
public class DestinationCoordinationWithCliques2 extends Module {
    ...
    public DestinationCoordinationWithCliques2(DataSet dataSet) {
        super(dataSet);
    }
    ...
    private void destinationCoordination() {
        ...
        int coordinatedTripCounts = dataSet.getTrips().values().stream().filter(mitoTrip -> mitoTrip.getCoordinatedTripId()>0).collect(Collectors.toList()).size();
    ...
    public void coordinateTripsForEgoIds(List<Integer> egoIds){
        ...
        for (Integer ego : egoIds) {
            List<MitoTrip> egoTrips = dataSet.getPersons().get(ego).getTrips();
            ...
            for (MitoTrip egoTrip : egoTrips) {
                ...
                trip.arrivalDay = egoTrip.getArrivalDay();
...
/*        float probability;
    ...
    public void recurseUpdateOriginalEgoTrips(List<Integer> egoList, List<Trip> trips, int storeTripIndex, int recurseTripIndex) {
        ...
        if (trip.precombinedTripIndexA >= 0) {
            ...
            List<MitoTrip> egoTrips = dataSet.getPersons().get(egoId).getTrips();
            ...
            for (MitoTrip egoTrip : egoTrips) {
                if (egoTripIndex == 0) {
                    ...
                    egoTrip.setArrivalDay(storeTrip.arrivalDay);

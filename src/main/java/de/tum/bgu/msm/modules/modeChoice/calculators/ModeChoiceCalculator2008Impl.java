...
import static de.tum.bgu.msm.data.Mode.walk;

public class ModeChoiceCalculator2008Impl extends ModeChoiceCalculator {
    ...
    private void setNests(Map<Mode, Map<String, Double>> coefficients) {
        ...
        nests.add(new Tuple<>(EnumSet.of(train, tramOrMetro, bus), coefficients.get(train).get("nestingCoefficient")));
    ...
    public EnumMap<Mode, Double> calculateUtilities(Purpose purpose, MitoHousehold household, MitoPerson person, MitoZone originZone, MitoZone destinationZone, TravelTimes travelTimes, double travelDistanceAuto, double travelDistanceNMT, double peakHour_s) {
        ...
        EnumSet<Mode> availableChoices = person.getModeRestriction().getRestrictedModeSet();
        availableChoices.remove(taxi);
    ...
    private static boolean isMotorized(Mode mode) {
        return !mode.equals(Mode.walk) && !mode.equals(Mode.bicycle);
    }
}

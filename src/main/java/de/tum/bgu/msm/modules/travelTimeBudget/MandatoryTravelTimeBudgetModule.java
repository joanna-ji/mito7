...
public class MandatoryTravelTimeBudgetModule extends Module {
    ...
    private void calculateTravelTimeBudgets() {
        ...
        results.add(service.submit(new MandatoryBudgetCalculator(dataSet.getModelledHouseholds().values(), Purpose.HBW, dataSet.getTravelTimes(), dataSet.getPeakHour())));
        results.add(service.submit(new MandatoryBudgetCalculator(dataSet.getModelledHouseholds().values(), Purpose.HBE, dataSet.getTravelTimes(), dataSet.getPeakHour())));
        service.shutdown();
        results.forEach(r -> {
            try {
                r.get();
